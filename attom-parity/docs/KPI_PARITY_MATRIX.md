...
| A8 | Nearby places / POI | §11-14 | **HAVE (verify after cache refill)** | Lane D shipped a993c776 (Sep 11 4:23 PM ET): `fetchNearbyPlaces` (OSM Overpass, 1 mi, 8 classes, 30-day cache `context_layer_cache` layer `poi`), composer `context_layers.nearby_places`, pdf.js one row per class; HTML twin mirrored 02a8938d. **Defect found on verification (E7):** the query was node-only and matched `amenity=supermarket` (tag does not exist) → grocery always 0, way-mapped hospitals/schools/parks missed, "none within 1 mi" printed as fact for Jacksonville 32256. Fixed 2b22f957 (`nwr` + `shop=supermarket` + `out center`), stale poi cache rows deleted | Fixed query verified against live OSM 7:15 PM ET from the same Jacksonville point at 3 mi: lane D's original returned **0** grocery; `shop=supermarket` node-only returned 8; `nwr` + `out center` returned 9 grocery (+ Walmart Supercenter mapped as a way) and 61 elements total — 10 grocery, 8 schools (2 ways), 4 parks (2 ways), 5 clinics, 4 pharmacies, 30 ramps, every way carrying a centroid. Remaining check: eyeball the first live report's rows |
...
|---|---|---|---|
| §ML SIGNAL$ Models | **OURS — foreclosure WITHHELD · tax deed PRINTS** | `shapira_model_validations` 2026-09-11 7:10 PM ET: foreclosure — V4 prod OOT AUC **0.478** (below the 0.547 baseline), v5b 0.63, v5 logreg 0.59, none passed → WITHHELD; tax deed — `td-soldvred-v1` (P(sold to third party) vs redeemed, task redefined by Ariel "B it is") **passed**: OOT AUC 0.8625 vs baseline 0.4841, n_test 1,624, Brier 0.117, validated 5:41 PM ET; composer 74ca54fc scores TD reports from it behind a sale-type-aware gate (f45b9aef) and the kill-list gate (0fceac18 pdf.js / a9cf3972 twin) | print model_version + gate status; a number prints only through the validation gate. FC rebuild #20240 |
